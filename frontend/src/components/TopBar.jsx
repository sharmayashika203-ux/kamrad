...
export default function TopBar({ onOpenAuth, currency, setCurrency }) {
  ...
  return (
    <div style={{
      ...
      <div className={`container ${isMobile ? 'topbar-mobile-marquee' : ''}`} style={
        !isMobile ? {
          display: 'flex',
          alignItems: 'center',
        ...
        <div className="marquee-content" style={!isMobile ? { display: 'flex', width: '100%', justifyContent: 'space-between' } : {}}>
          {isMobile ? (
            <>
              <div className="marquee-group">{renderContent()}</div>
              <div className="marquee-group">{renderContent()}</div>
            </>
          ) : (
            renderContent()
